...
async function start() {
  ...
  const desc = await page.$eval('section.product-features', el => el.innerHTML.trim()).catch(() => '');
  const desc2 = await page.$$eval('div.product-description-short', els => els ? els.innerHTML.trim() : '').catch(() => '');
