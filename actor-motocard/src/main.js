...
async function start() {
  ...
  const description = await page.$eval('#prod-desc-body', el => el.innerText.trim()).catch(() => '');
  const desc = await page.$eval('table.data-sheet__table', el => el.innerHTML.trim()).catch(() => '');
  ...
  const startUrls = queries.map(producto => ({
    url: `https://www.motocard.com/search?q=${encodeURIComponent(producto.modelo)}&p=1&nidx`,
